...
- 新增分层显示说明：支持按 X 轴切片、Y 轴水平层和材料顺序查看投影，并支持全部、单层、向上或向下范围显示。
- 辅助放置页新增“自动清除阻挡方块”选项，默认关闭；开启后仅通过原版破坏流程清除投影缺失位置上的实际方块。

### Fixed

- 修复分层显示仍会生成隐藏方块网格的问题，现在隐藏层不会继续渲染。
- 修复观察者、活塞、粘性活塞、投掷器、发射器和漏斗自动放置时方向可能错误的问题；无法由当前放置角度得到目标方向时会跳过放置。
- 水和熔岩投影现在可按水桶和熔岩桶参与辅助放置。
- 自动清除阻挡方块前会检查玩家手持物品和原版破坏速度，空手或无法破坏目标时不会发送破坏请求。
